	...
	override fun dispatchTouchEvent(ev:MotionEvent?):Boolean {
		if(directionRight) {
			if(isDrawerOpen(getChildAt(1)) || isDrawerVisible(getChildAt(1))) {
				ev?.let {
					return if(ev.x <= getChildAt(1).right) {
						false
					}
					else (ev.x >= getChildAt(1).right && ev.y >= height - (getChildAt(1).right * 4 / 5))
			...
			ev?.let {
				if(isDrawerOpen(getChildAt(1)) || isDrawerVisible(getChildAt(1))) {
					...
					else (ev.x <= getChildAt(1).left && ev.y >= height - (getChildAt(1).left) * 4f / 5f)
				}
			}
		}
